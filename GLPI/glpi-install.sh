...
while true; do
    read -p -r "Detected Timezone is [$DETECTED_TZ]. Accept? (y/n): " TZ_CONFIRM
    if [[ $TZ_CONFIRM =~ ^[Yy]$ ]]; then
        TIMEZONE=$DETECTED_TZ
        break
    elif [[ $TZ_CONFIRM =~ ^[Nn]$ ]]; then
        read -p -r "Enter your specific Timezone (e.g., America/New_York): " TIMEZONE
        [ -n "$TIMEZONE" ] && break
        read -p -r "Enter your specific Timezone (e.g., America/New_York). See /usr/share/zoneinfo/ for valid options: " TIMEZONE
...
# --- DOMAIN ---
while true; do
    read -p -r "Enter Domain/Hostname to be used in Apache configuration (Type 'localhost' for local setup): " DOMAIN_NAME
    [ -n "$DOMAIN_NAME" ] && break
...
# --- DB USERNAME ---
while true; do
    read -p -r "Enter Database Username (e.g., glpi) [will be created if it does not exist]: " DB_USER
    [ -n "$DB_USER" ] && break
...
# --- DB NAME ---
while true; do
    read -p -r "Enter Database Name (e.g., glpidb) [will be created if it does not exist]: " DB_NAME
    [ -n "$DB_NAME" ] && break
...
while true; do
    ...
    read -s -p -r "Enter Password: " DB_PASS
    echo ""
    if [ -z "$DB_PASS" ]; then
        echo "Password cannot be empty!"
        continue
    fi
    read -s -p -r "Confirm Password: " DB_PASS_CONFIRM
    echo ""
    ...
    if [ "$DB_PASS" != "$DB_PASS_CONFIRM" ]; then
        ...
        read -p -r "Continue with this password? (y/n): " PASS_WEAK_CONFIRM
...
echo "----------------------------------------------------------"
read -p -r "Apply settings and install GLPI now? (y/n): " FINAL_CONFIRM
